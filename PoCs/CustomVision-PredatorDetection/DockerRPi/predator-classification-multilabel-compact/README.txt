...
```bash
docker run -p 127.0.0.1:81:80 -d <your image name>
```

Then use your favorite tool to connect to the end points.

POST http://127.0.0.1:81/image with multipart/form-data using the imageData key
e.g
    curl -X POST http://127.0.0.1:81/image -F imageData=@some_file_name.jpg
